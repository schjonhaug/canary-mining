...
use crate::{
    template_distribution_protocol::template_data::{
        TemplateData, template_status_snapshot_from_ipc,
    },
    template_runtime_status::{TemplateRuntimeStatus, TemplateStatusSource},
};
...
const WAIT_NEXT_TIMEOUT_MILLIS: f64 = 1_000.0;
const MAX_RETAINED_TEMPLATES_PER_PREV_HASH: usize = 256;
...
pub struct BitcoinCoreSv2TDP {
    ...
    template_runtime_status: TemplateRuntimeStatus,
...
impl BitcoinCoreSv2TDP {
    ...
    async fn fetch_template_data(
        ...
        let status_snapshot = match template_status_snapshot_from_ipc(
            &template_ipc_client,
            thread_ipc_client.clone(),
        ...
        .await
        {
            ...
            Err(error) => {
                tracing::warn!("Failed to build IPC template status snapshot: {error}");
                None
            }
        };
    ...
    async fn publish_template(
        ...
        if let Some(snapshot) = template_data.status_snapshot(source) {
            self.template_runtime_status.set_template(snapshot);
        }
        ...
        self.prune_template_data_for_prev_hash(template_data.get_prev_hash())
            .await;

        Ok(())
    }

    async fn prune_template_data_for_prev_hash(&self, prev_hash: U256<'static>) {
        let removed_template_data = {
            let mut template_data_guard = match self.template_data.write() {
                Ok(guard) => guard,
                Err(e) => {
                    tracing::warn!("Failed to acquire write lock on template_data: {:?}", e);
                    return;
                }
            };

            let template_ids = template_data_guard
                .values()
                .filter(|template_data| template_data.get_prev_hash() == prev_hash)
                .map(TemplateData::get_template_id)
                .collect::<Vec<_>>();
            let evicted_template_ids = oldest_template_ids_to_evict(
                template_ids,
                MAX_RETAINED_TEMPLATES_PER_PREV_HASH,
            );

            if evicted_template_ids.is_empty() {
                tracing::debug!(
                    "Template data cache has {} entries; no same-tip templates evicted",
                    template_data_guard.len()
                );
                Vec::new()
            } else {
                let mut removed_template_data = Vec::with_capacity(evicted_template_ids.len());
                for template_id in &evicted_template_ids {
                    if let Some(template_data) = template_data_guard.remove(template_id) {
                        removed_template_data.push(template_data);
                    }
                }
                tracing::debug!(
                    "Evicted {} same-tip template data entries; cache now has {} entries",
                    removed_template_data.len(),
                    template_data_guard.len()
                );
        ...
        self.destroy_removed_template_data(removed_template_data, false)
            .await;
    }

    async fn destroy_removed_template_data(
        &self,
        removed_template_data: Vec<TemplateData>,
        cancel_on_error: bool,
    ) {
        if removed_template_data.is_empty() {
            return;
        }

        tracing::debug!("Creating a dedicated thread IPC client for destroy_ipc_client");
        let thread_ipc_client = match self.new_thread_ipc_client().await {
            Ok(thread_ipc_client) => thread_ipc_client,
            Err(e) => {
                tracing::warn!(
                    "Failed to create thread IPC client for template cleanup: {:?}",
                    e
                );
                if cancel_on_error {
                    tracing::warn!("Terminating Sv2 Bitcoin Core IPC Connection");
        ...
        for template_data in removed_template_data {
            if let Err(e) = template_data
                ...
                .await
            {
                tracing::warn!("Failed to destroy template IPC client: {:?}", e);
                if cancel_on_error {
                    tracing::warn!("Terminating Sv2 Bitcoin Core IPC Connection");
                    self.global_cancellation_token.cancel();
                    return;
    ...
    async fn bootstrap_template_ipc_client_from_coinbase_output_constraints(
        ...
        self.publish_template(
            template_data,
            TemplateStatusSource::IpcBootstrap,
            true,
            true,
            true,
        )
    ...
    async fn new_wait_next_request(
        ...
        // Keep each waitNext attachment short. Bitcoin Core may rebuild candidate templates while
        // waiting, so the outer monitor loop controls the long-term cadence with min_interval.
        wait_next_request_options.set_timeout(WAIT_NEXT_TIMEOUT_MILLIS);
    ...
    async fn process_stale_template_data(&self, stale_template_ids: HashSet<u64>) {
        ...
        tokio::task::spawn_local(async move {
            ...
            self_clone
                .destroy_removed_template_data(removed_template_data, true)
                .await;
        });
    }
}

fn oldest_template_ids_to_evict(
    mut template_ids: Vec<u64>,
    max_retained_templates: usize,
) -> Vec<u64> {
    if template_ids.len() <= max_retained_templates {
        return Vec::new();
    }

    template_ids.sort_unstable();
    template_ids.truncate(template_ids.len() - max_retained_templates);
    template_ids
}
...
mod tests {
    ...
    #[test]
    fn oldest_template_ids_to_evict_keeps_cache_under_limit() {
        let evicted = oldest_template_ids_to_evict(vec![4, 1, 3, 2], 2);

        assert_eq!(evicted, vec![1, 2]);
    }

    #[test]
    fn oldest_template_ids_to_evict_keeps_exact_limit() {
        let evicted = oldest_template_ids_to_evict(vec![3, 1, 2], 3);

        assert!(evicted.is_empty());
    }

    #[test]
    fn oldest_template_ids_to_evict_can_evict_all_entries() {
        let evicted = oldest_template_ids_to_evict(vec![2, 1, 3], 0);

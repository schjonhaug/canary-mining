use crate::{
    template_distribution_protocol::error::TemplateDataError,
    template_runtime_status::{TemplateStatusKind, TemplateStatusSnapshot, TemplateStatusSource},
};
...
use stratum_core::bitcoin::{
    ScriptBuf, Target, Transaction, TxOut,
...
pub struct TemplateData {
    ...
    status_snapshot: Option<TemplateStatusSnapshot>,
}
...
impl TemplateData {
    ...
    ) -> Option<TemplateStatusSnapshot> {
        let mut snapshot = self.status_snapshot.clone()?;
        snapshot.source = source;
        snapshot.status = TemplateStatusKind::Available;
        Some(snapshot)
    }
...
pub(crate) async fn template_status_snapshot_from_ipc(
    template_ipc_client: &BlockTemplateIpcClient,
    thread_ipc_client: ThreadIpcClient,
    block_reward_remaining: u64,
    coinbase_tx: &Transaction,
) -> Result<Option<TemplateStatusSnapshot>, TemplateDataError> {
    ...
    let template_block_bytes = template_block_response.get()?.get_result()?;
    let block: Block = deserialize(template_block_bytes)?;
    let Some(height) = block_height_from_coinbase_script_sig(&coinbase_tx.input[0].script_sig)
    else {
        tracing::warn!("Could not parse block height from IPC template coinbase script");
        return Ok(None);
    };

    let required_output_sats = coinbase_tx.output.iter().map(|output| output.value.to_sat()).sum::<u64>();
    let reward_sats = block_reward_remaining.saturating_add(required_output_sats);
    let subsidy_sats = block_subsidy_sats(height);
    let fee_sats = reward_sats.checked_sub(subsidy_sats);
    let weight = block.weight().to_wu();

    Ok(Some(TemplateStatusSnapshot {
        height,
        reward_sats: Some(reward_sats),
        fee_sats,
        weight: Some(weight),
        weight_percent: Some((weight as f64 / 4_000_000.0) * 100.0),
        transaction_count: block.txdata.len().saturating_sub(1),
        updated_at: 0,
        source: TemplateStatusSource::IpcBootstrap,
        status: TemplateStatusKind::Available,
    }))
}

fn block_height_from_coinbase_script_sig(script_sig: &ScriptBuf) -> Option<u64> {

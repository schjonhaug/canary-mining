...
use crate::{
    channel_manager::{
        job_store::BoundedJobStore, ChannelManager, RouteMessageTo, CLIENT_SEARCH_SPACE_BYTES,
    },
    error::{self, PoolError, PoolErrorKind},
    miner_stats::MinerChannelKind,
...
impl HandleMiningMessagesFromClientAsync for ChannelManager {
            ...
            .super_safe_lock(|channel_manager_data| {
                ...
                downstream
                    ...
                    .super_safe_lock(|downstream_data| {
                        if downstream_data.standard_channels.remove(&msg.channel_id).is_some() {
                            self.miner_stats.record_channel_closed(
                                downstream_id,
                                msg.channel_id,
                                MinerChannelKind::Standard,
                            );
                        }
                        if downstream_data.extended_channels.remove(&msg.channel_id).is_some() {
                            self.miner_stats.record_channel_closed(
                                downstream_id,
                                msg.channel_id,
                                MinerChannelKind::Extended,
                            );
                        }
                    });
    ...
    async fn handle_open_standard_mining_channel(
        ...
        let messages = self.channel_manager_data.super_safe_lock(|channel_manager_data| {
            ...
            let payout_mode = match PayoutMode::try_from_user_identity(
            ...
            downstream.downstream_data.super_safe_lock(|downstream_data| {
                ...
                let job_store = BoundedJobStore::new();
                ...
                self.miner_stats.record_channel_opened(
                    downstream_id,
                    channel_id,
                    MinerChannelKind::Standard,
                    user_identity.to_string(),
                    nominal_hash_rate.into(),
                );
    ...
    async fn handle_submit_shares_standard(
        ...
        debug!("Received SubmitSharesStandard: {msg}");
        let downstream_id =
            client_id.expect("client_id must be present for downstream_id extraction");
        ...
        let messages = self.channel_manager_data.super_safe_lock(|channel_manager_data| {
            ...
            downstream.downstream_data.super_safe_lock(|downstream_data| {
                ...
                self.miner_stats.record_share_seen(
                    downstream_id,
                    channel_id,
                    MinerChannelKind::Standard,
                );
                ...
                match res {
                    Ok(ShareValidationResult::Valid(share_hash)) => {
                        ...
                        self.miner_stats.record_share_accepted(
                            downstream_id,
                            channel_id,
                            MinerChannelKind::Standard,
                            share_accounting.get_shares_accepted().into(),
                            share_accounting.get_best_diff(),
                            share_accounting.get_blocks_found().into(),
                        );
                    ...
                    Ok(ShareValidationResult::BlockFound(share_hash, template_id, coinbase)) => {
                        ...
                        self.miner_stats.record_block_found(
                            downstream_id,
                            channel_id,
                            MinerChannelKind::Standard,
                    ...
                    Err(ShareValidationError::Invalid) => {
                        self.miner_stats.record_share_rejected(
                            downstream_id,
                            channel_id,
                            MinerChannelKind::Standard,
                            "invalid-share",
    ...
    async fn handle_submit_shares_extended(
        ...
        debug!("Received SubmitSharesExtended: {msg}");
        let downstream_id =
            client_id.expect("client_id must be present for downstream_id extraction");

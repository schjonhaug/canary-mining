    ...
    stratum_core::{
        ...
        channels_sv2::{
            ...
            server::{
                ...
                group::GroupChannel,
                jobs::{extended::ExtendedJob, standard::StandardJob},
...
impl ChannelManager {
    ...
    pub async fn new(
        ...
        let channel_manager = ChannelManager {
            ...
            bitcoin_network: match config.template_provider_type() {
                TemplateProviderType::BitcoinCoreIpc { network, .. } => Some(network.clone()),
    ...
    ) -> Option<GroupChannel<'static, BoundedJobStore<ExtendedJob<'static>>>> {
                        ...
                        match res {
                            Ok((stream, socket_address)) => {
                                ...
                                task_manager_clone.spawn(async move {
                                    ...
                                    this.template_runtime_status
                                        .miner_activity()
                                        .increment_active_downstreams();
    ...
    pub fn remove_downstream(&self, downstream_id: DownstreamId) {
        self.miner_stats.record_downstream_closed(downstream_id);
        let removed = self.channel_manager_data.super_safe_lock(|cm_data| {
            let removed = cm_data.downstream.remove(&downstream_id).is_some();
            cm_data
                .vardiff
        ...
        if removed {
            self.template_runtime_status
                .miner_activity()
                .decrement_active_downstreams();
        }
    ...
    fn run_vardiff_on_extended_channel(
        ...
        channel_id: ChannelId,
        channel_state: &mut ExtendedChannel<'static, BoundedJobStore<ExtendedJob<'static>>>,
    ...
    fn run_vardiff_on_standard_channel(
        downstream_id: DownstreamId,
        channel_id: ChannelId,
        channel: &mut StandardChannel<'static, BoundedJobStore<StandardJob<'static>>>,
        vardiff_state: &mut VardiffState,
        updates: &mut Vec<RouteMessageTo>,
    ) {

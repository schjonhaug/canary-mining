...
use crate::{
    ...
    channel_manager::job_store::BoundedJobStore,
    utils::PayoutMode,
};
...
pub struct DownstreamData {
    pub group_channel: GroupChannel<'static, BoundedJobStore<ExtendedJob<'static>>>,
    pub extended_channels:
        HashMap<ChannelId, ExtendedChannel<'static, BoundedJobStore<ExtendedJob<'static>>>>,
    pub standard_channels:
        HashMap<ChannelId, StandardChannel<'static, BoundedJobStore<StandardJob<'static>>>>,
...
impl Downstream {
    fn handle_error_action(
        ...
        match e.action {
            Action::Log => {
                if e.kind.is_expected_downstream_close() {
                    debug!(
                        downstream_id = self.downstream_id,
                        error_kind = ?e.kind,
                        "{context} returned after downstream closed"
                    );
                } else {
                    warn!(
                        downstream_id = self.downstream_id,
            ...
            Action::Disconnect(_) => {
                if e.kind.is_expected_downstream_close() {
                    debug!(
                        downstream_id = self.downstream_id,
                        error_kind = ?e.kind,
                        "{context} observed downstream close; cancelling downstream token"
                    );
                } else {
                    warn!(
                        downstream_id = self.downstream_id,
    ...
    pub fn new(
        ...
        group_channel: GroupChannel<'static, BoundedJobStore<ExtendedJob<'static>>>,
        channel_manager_sender: Sender<(DownstreamId, Mining<'static>, Option<Vec<Tlv>>)>,
        channel_manager_receiver: Receiver<(Mining<'static>, Option<Vec<Tlv>>)>,
    ...
    pub async fn start(
        ...
        task_manager.spawn(async move {
            loop {
                ...
                tokio::select! {
                    ...
                    res = self_clone_1.handle_downstream_message() => {
                        if let Err(e) = res {
                            if e.kind.is_expected_downstream_close() {
                                debug!(?e, "Downstream {downstream_id} closed connection");
                            } else {
                    ...
                    res = self_clone_2.handle_channel_manager_message() => {
                        if let Err(e) = res {
                            if e.kind.is_expected_downstream_close() {
                                debug!(?e, "Channel manager path closed for downstream {downstream_id}");
                            } else {
            ...
            self.downstream_io.close();
            on_disconnect(self.downstream_id);
            debug!("Downstream: unified message loop exited.");
        });
    }

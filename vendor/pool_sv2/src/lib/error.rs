...
impl PoolErrorKind {
    pub(crate) fn is_expected_downstream_close(&self) -> bool {
        match self {
            PoolErrorKind::ChannelRecv(_) => true,
            PoolErrorKind::Io(error) => matches!(
                error.kind(),
            ...
            PoolErrorKind::Framing(error) => {
                let debug = format!("{error:?}");
                debug.contains("SocketClosed") || debug.contains("UnexpectedEof")
            }
            PoolErrorKind::Noise(error) => {
                let debug = format!("{error:?}");
                debug.contains("SocketClosed") || debug.contains("UnexpectedEof")
            }
            _ => false,
